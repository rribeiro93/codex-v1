...
import { MonthlySummaryEntry, MonthlyTransaction } from '../../types';
...
export type TransactionMappingUpdates = Partial<
  Pick<MonthlyTransaction, 'category' | 'cleanName' | 'mappingId' | 'mappingTransaction'>
>;
...
export interface UseMonthlySummaryResult {
  ...
  handleBarClick: (barData: unknown) => void;
  loadTransactions: (monthValue: string, displayLabel?: string) => void;
  sortedTransactions: MonthlyTransaction[];
  sortColumn: TransactionSortColumn | '';
  sortDirection: 'asc' | 'desc';
  handleSort: (columnKey: TransactionSortColumn) => void;
  handleTransactionMappingChange: (transactionId: string, updates?: TransactionMappingUpdates) => void;
  yearlyAverage: number;
  transactionsTotalAmount: number;
  installmentStats: { count: number; total: number };
}
...
function translateCompositeLabel(label: string): string {
  const trimmed = label.trim();
  if (!trimmed) {
    return trimmed;
  }
...
const resolveSortValue = (
  item: MonthlyTransaction,
  columnKey: TransactionSortColumn
): number | string | null => {
  switch (columnKey) {
    case 'date': {
...
export function useMonthlySummary(): UseMonthlySummaryResult {
  const [state, setState] = useState<MonthlySummaryState>(initialState);
  ...
  const loadSummary = useCallback(
    async (year?: string) => {
      ...
      try {
        ...
        const normalized: MonthlySummaryEntry[] = summary.map((item) => {
  ...
  const loadTransactions = useCallback(
    async (monthValue: string, displayLabel?: string) => {
      if (typeof fetch !== 'function') {
        return;
      }
      ...
      try {
        ...
        const normalizedTransactions: MonthlyTransaction[] = payloadTransactions.map(
          (transaction: Record<string, any>, index: number) => {
            ...
            const fileName =
              typeof transaction?.fileName === 'string' ? transaction.fileName : '';
            const mappingId =
            ...
            const normalizedInstallments: MonthlyTransaction['installments'] = (() => {
  ...
  const handleBarClick = useCallback(
    (barData: unknown) => {
      ...
      const payload =
        'payload' in barData && typeof (barData as { payload?: unknown }).payload === 'object'
  ...
  const handleTransactionMappingChange = useCallback(
    (transactionId: string, updates: TransactionMappingUpdates = {}) => {
      if (!transactionId) {
        return;
      }
  ...
  const sortedTransactions = useMemo<MonthlyTransaction[]>(() => {
    if (!sortColumn) {
      return transactions;
    }
  ...
  const handleSort = useCallback((columnKey: TransactionSortColumn) => {
    setState((prev) => {
