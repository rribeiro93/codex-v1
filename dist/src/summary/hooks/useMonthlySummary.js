...
exports.useMonthlySummary = useMonthlySummary;
...
const resolveSortValue = (item, columnKey) => {
    switch (columnKey) {
        ...
        case 'place': {
            ...
            const place = cleanName || (typeof item.place === 'string' ? item.place.trim() : '');
            if (!place) {
                return null;
            }
...
function useMonthlySummary() {
    ...
    const { data, years, selectedYear, selectedMonth, selectedMonthLabel, transactions, isLoading, isTransactionsLoading, error, transactionsError, sortColumn, sortDirection, yearlyAverage } = state;
    ...
    const loadSummary = (0, react_1.useCallback)(async (year) => {
        ...
        try {
            ...
            const normalized = summary.map((item) => {
                ...
                const monthName = typeof item.monthName === 'string' && item.monthName ? item.monthName : '';
                const totalAmount = typeof item.totalAmount === 'number' && Number.isFinite(item.totalAmount)
                    ? item.totalAmount
                    : 0;
                const installmentAmount = typeof item.installmentAmount === 'number' && Number.isFinite(item.installmentAmount)
                    ? item.installmentAmount
                    : 0;
                const nonInstallmentAmount = typeof item.nonInstallmentAmount === 'number' &&
                ...
                const displayMonth = monthName
                    ? translateMonthLabel(monthName)
                    : translateCompositeLabel(month);
                return {
                    month,
                    monthName,
                    displayMonth,
            ...
            const averageTotal = normalized.length > 0
    ...
    const loadTransactions = (0, react_1.useCallback)(async (monthValue, displayLabel) => {
        if (typeof fetch !== 'function') {
            return;
        }
        ...
        try {
            const response = await fetch(`/api/statements/transactions?month=${encodeURIComponent(targetMonth)}`);
            if (!response.ok) {
                throw new Error(`Failed with status ${response.status}`);
            }
            const body = await response.json();
            const payloadTransactions = Array.isArray(body.transactions) ? body.transactions : [];
            const normalizedMonthName = typeof body.monthName === 'string' && body.monthName.trim()
                ? body.monthName.trim()
                : '';
            const normalizedTransactions = payloadTransactions.map((transaction, index) => {
                ...
                const cleanName = typeof transaction?.cleanName === 'string' ? transaction.cleanName : '';
                ...
                const amount = typeof transaction?.amount === 'number' && Number.isFinite(transaction.amount)
                    ? transaction.amount
                    : 0;
                const statementId = typeof transaction?.statementId === 'string' ? transaction.statementId : '';
                const fileName = typeof transaction?.fileName === 'string' ? transaction.fileName : '';
                const mappingId = typeof transaction?.mappingId === 'string' ? transaction.mappingId : '';
                const mappingTransaction = typeof transaction?.mappingTransaction === 'string'
            ...
            const finalLabel = selectedYear && localizedBase && !localizedBase.includes(selectedYear)
    ...
    const handleBarClick = (0, react_1.useCallback)((barData) => {
        if (!barData || typeof barData !== 'object') {
            return;
        }
        const payload = 'payload' in barData && typeof barData.payload === 'object'
            ? barData.payload
            : barData;
        if (!payload) {
            return;
        }
        const monthValue = typeof payload.month === 'string' && payload.month ? payload.month : '';
        if (!monthValue) {
            return;
        }
        ...
        const display = (typeof payload.displayMonth === 'string' && payload.displayMonth) ||
            (typeof payload.monthName === 'string' && payload.monthName) ||
            monthValue;
        const baseLabel = selectedYear && display && !display.includes(selectedYear)
    ...
    const handleTransactionMappingChange = (0, react_1.useCallback)((transactionId, updates = {}) => {
        if (!transactionId) {
            return;
        }
        setState((prev) => {
    ...
    const installmentStats = (0, react_1.useMemo)(() => {
    ...
    const transactionsTotalAmount = (0, react_1.useMemo)(() => {
        if (!transactions.length) {
            return 0;
        }
