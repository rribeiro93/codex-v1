...
exports.handleUpdateSinglePlaceCategory = handleUpdateSinglePlaceCategory;
...
async function handleUpdateSinglePlaceCategory(req, res) {
    ...
    const transactionValue = typeof req.body?.transaction === 'string' ? req.body.transaction.trim() : '';
    ...
    const categoryValue = typeof rawCategoryValue === 'string' ? rawCategoryValue.trim() : undefined;
    const cleanNameValue = typeof rawCleanNameValue === 'string' ? rawCleanNameValue.trim() : undefined;
    ...
    if (idValue) {
        try {
            filter = { _id: new mongodb_1.ObjectId(idValue) };
        }
        catch (error) {
    ...
    try {
        const result = await db.collection('transaction_mappings').findOneAndUpdate(filter, updatePayload, {
        ...
        if (!resolvedDocument) {
            ...
            const upsertedId = result?.lastErrorObject?.upserted ?? result?.lastErrorObject?.upsertedId ?? null;
            if (upsertedId) {
                const resolvedId = typeof upsertedId === 'object' && upsertedId !== null ? upsertedId : new mongodb_1.ObjectId(upsertedId);
